...
for NN in range(NNN+1):
    try:
        loss[NN] = np.loadtxt("itint_rand_cev_res_" + str(NN) + ".csv", delimiter = ";")
        comp[NN] = np.loadtxt("itint_rand_cev_cmp_" + str(NN) + ".csv", delimiter = ";")
        Ypred[NN] = np.loadtxt("itint_rand_cev_ypr_" + str(NN) + ".csv", delimiter = ";")
        hpred = np.loadtxt("itint_rand_cev_hpr_" + str(NN) + ".csv", delimiter = ";")
...
Y = np.loadtxt("itint_hedg_cev_y.csv", delimiter = ";")
Y = np.reshape(Y, (-1, 1))
...
ax2.set_ylim([0.0005, vmax2])
ax1.set_xlabel("N")
ax1.set_ylabel("MSE of option") 
ax2.set_ylabel("IMSE of strategy (dashed)") 
plt.legend(loc = "upper right", ncol = 2)
plt.savefig("itint_rand_cev_training.png", bbox_inches = 'tight') 
plt.close(fig)
...
ax = Axes3D(fig)
for NN in range(NNN+1):
    hist, bins = np.histogram(Ypred[NN], bins = 8, range = (0, 9))
    hist = hist/np.sum(hist)
    xs = (bins[:-1] + bins[1:])/2
    ax.bar(xs, hist, zs = NN, zdir = 'y', width = 0.27, color = cmap(NN/NNN), alpha = 0.8, label = "N = " + str(NN))

hist, bins = np.histogram(Y[Mtrain:, 0], bins = 8, range = (0, 9))
hist = hist/np.sum(hist)
ax.bar(xs, hist, zs = NNN+1, zdir = 'y', width = 0.27, color = 'black', alpha = 0.8, label = "True")
ax.set_xlabel('G')
...
plt.savefig("itint_rand_cev_testdistr.png", bbox_inches = 'tight')
plt.close(fig)
...
plt.legend(loc = "upper right", ncol = 2)
plt.savefig("itint_rand_cev_comp.png", bbox_inches = 'tight') 
plt.close(fig)
...
axes.set_ylim([-1.1, 0.3])
axes.set_xlabel("Time")
axes.set_ylabel("Number of shares") 
axes.yaxis.set_label_position("right")
axes.yaxis.tick_right()
plt.legend(loc = "upper right", ncol = 4)
plt.savefig("itint_rand_cev_hedgetest.png", bbox_inches = 'tight') 
plt.close(fig)