...
X = np.loadtxt("itint_hedg_bm_x.csv", delimiter = ";", dtype = np.float32)
X = np.reshape(X, (M, n, d))
Y = np.loadtxt("itint_hedg_bm_y.csv", delimiter = ";", dtype = np.float32)
Y = np.reshape(Y, (-1, 1))
...
if NN == 0:
    ...
    for N in range(NN):
        ...
        Qg = np.sum(np.tile(np.expand_dims(np.square(output1[N, :-1, 0])*dt, axis = 1), (1, M, 1)), axis = 0)
    ...
    for N in range(NN):
        ...
        Qg = np.cumsum(np.tile(np.expand_dims(np.square(output1[N, :-1, 0])*dt, axis = 1), (1, M, 1)), axis = 0)
...
K = -0.5
Htrue = np.zeros([M, n-1, d])
for t in range(n-1):
    Htrue[:, t, :] = sst.norm.cdf((X[:, t, :]-K)/np.sqrt(T-tt[t]))
...
np.savetxt("itint_rand_bm_res_" + str(NN) + ".csv", loss, delimiter = ";")
np.savetxt("itint_rand_bm_cmp_" + str(NN) + ".csv", comp, delimiter = ";")

np.savetxt("itint_rand_bm_ypr_" + str(NN) + ".csv", Ypred[Mtrain:], delimiter = ";")
np.savetxt("itint_rand_bm_hpr_" + str(NN) + ".csv", np.reshape(Hpred[Mtrain:], (M-Mtrain, (n-1)*d)), delimiter = ";")