...
for n in range(N+1):
    try:
        loss[n] = np.loadtxt("bm/bm_res_" + str(n) + ".csv")
        comp[n] = np.loadtxt("bm/bm_cmp_" + str(n) + ".csv")
        Gpred[n] = np.loadtxt("bm/bm_Gpr_" + str(n) + ".csv")
        Hpred[n] = np.reshape(np.loadtxt("bm/bm_Hpr_" + str(n) + ".csv"), [L, K-1, d])
        print("Tables for n = " + str(n) + " loaded")
    except:
        print("Tables for n = " + str(n) + " not available")
        
X = np.reshape(np.loadtxt("bm/bm_X.csv"), [L, K, d])
G = np.loadtxt("bm/bm_G.csv")

K_strike = -0.5
Htrue = np.zeros([L, K-1, d])
for k in range(K-1):
    Htrue[:, k] = sst.norm.cdf((X[:, k]-K_strike)/np.sqrt(T-tt[k]))
