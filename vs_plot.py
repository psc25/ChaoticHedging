...
for N in range(NN+1):
    try:
        loss[N] = np.loadtxt("vs/vs_res_" + str(N) + ".csv", delimiter = ";")
        comp[N] = np.loadtxt("vs/vs_cmp_" + str(N) + ".csv", delimiter = ";")
        Gpred[N] = np.loadtxt("vs/vs_Gpr_" + str(N) + ".csv", delimiter = ";")
        Hpred[N] = np.loadtxt("vs/vs_Hpr_" + str(N) + ".csv", delimiter = ";")
        print("Tables for N = " + str(N) + " loaded")
    except:
        print("Tables for N = " + str(N) + " not available")

G = np.loadtxt("vs/vs_G.csv", delimiter = ";")

Htrue = np.loadtxt("vs/vs_strat.csv", delimiter = ";")
...
for N in range(NN+1):
    hist, bins = np.histogram(Gpred[N], bins = 8, range = (0, 4.0))
    hist = hist/np.sum(hist)
    x = (bins[:-1] + bins[1:])/2.0
    ax.bar3d(x, y = N, z = 0.0, dx = 0.09, dy = 0.09, dz = hist, color = cmap(N/NN), alpha = 0.8, label = "N = " + str(N))

hist, bins = np.histogram(G[Ltrain:], bins = 8, range = (0, 4.0))
hist = hist/np.sum(hist)
...
plt.savefig("vs/vs_test_distr.png", bbox_inches = 'tight')
plt.close(fig)
...
plt.legend(loc = "upper right", ncol = 2, prop={'size': 8})
plt.savefig("vs/vs_comp.png", bbox_inches = 'tight') 
plt.close(fig)
...
axes.set_ylim([-1.0, 0.25])
axes.set_xlabel("Time")
axes.set_ylabel("Number of shares")
axes.yaxis.set_label_position("right")
axes.yaxis.tick_right()
plt.legend(loc = "upper right", ncol = 4, prop={'size': 8})
plt.savefig("vs/vs_test_hedge.png", bbox_inches = 'tight') 
plt.close(fig)