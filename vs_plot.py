...
for n in range(N+1):
    try:
        loss[n] = np.loadtxt("vs/vs_res_" + str(n) + ".csv")
        comp[n] = np.loadtxt("vs/vs_cmp_" + str(n) + ".csv")
        Gpred[n] = np.loadtxt("vs/vs_Gpr_" + str(n) + ".csv")
        Hpred[n] = np.reshape(np.loadtxt("vs/vs_Hpr_" + str(n) + ".csv"), [L, K-1, d])
        print("Tables for n = " + str(n) + " loaded")
    except:
        print("Tables for n = " + str(n) + " not available")

X = np.reshape(np.loadtxt("vs/vs_X.csv"), [L, K, d])
G = np.loadtxt("vs/vs_G.csv")

Htrue = np.reshape(np.loadtxt("vs/vs_hedg.csv"), [L, K-1, d])
...
for n in range(1, N+1):
    plt.plot(tt[:-1], Hpred[n, indplot[0], :], linestyle = ":", color = cmap(n/N), label = "N = " + str(n))
    
for l in range(1, Lplot):
    plt.plot(tt[:-1], Htrue[indplot[l], :], "k-")
    for n in range(1, N+1):
        plt.plot(tt[:-1], Hpred[n, indplot[l], :], linestyle = ":", color = cmap(n/N))
