...
for NN in range(NNN+1):
    try:
        loss[NN] = np.loadtxt("itint_rand_am_res_" + str(NN) + ".csv", delimiter = ";")
        comp[NN] = np.loadtxt("itint_rand_am_cmp_" + str(NN) + ".csv", delimiter = ";")
        Ypred[NN] = np.loadtxt("itint_rand_am_ypr_" + str(NN) + ".csv", delimiter = ";")
        hpred = np.loadtxt("itint_rand_am_hpr_" + str(NN) + ".csv", delimiter = ";")
        Hpred[NN] = np.reshape(hpred, (M-Mtrain, n-1, d))
    except:
        print("Tables for NN = " + str(NN) + " not available yet")

X = np.loadtxt("itint_hedg_am_x.csv", delimiter = ";")
X = np.reshape(X, (M, n, d))
Y = np.loadtxt("itint_hedg_am_y.csv", delimiter = ";")
Y = np.reshape(Y, (-1, 1))
...
ax1.set_yscale("log")
ax2.set_yscale("log")
ax1.set_ylim([0.006, vmax1])
ax2.set_ylim([0.095, vmax2])
ax1.set_xlabel("N")
ax1.set_ylabel("MSE of option") 
ax2.set_ylabel("IMSE of strategy (dashed)") 
plt.legend(loc = "upper right", ncol = 2)
plt.savefig("itint_rand_am_training.png", bbox_inches = 'tight') 
plt.close(fig)
...
ax = Axes3D(fig)
for NN in range(NNN+1):
    hist, bins = np.histogram(Ypred[NN], bins = 8, range = (0, 6.0))
    xs = (bins[:-1] + bins[1:])/2
    hist = hist/np.sum(hist)
    ax.bar(xs, hist, zs = NN, zdir = 'y', width = 0.18, color = cmap(NN/NNN), alpha = 0.8, label = "N = " + str(NN))

hist, bins = np.histogram(Y[Mtrain:], bins = 8, range = (0, 6.0))
hist = hist/np.sum(hist)
ax.bar(xs, hist, zs = NNN+1, zdir = 'y', width = 0.18, color = 'black', alpha = 0.8, label = "True")
ax.set_xlabel('G')
...
plt.savefig("itint_rand_am_testdistr.png", bbox_inches = 'tight')
plt.close(fig)
...
plt.legend(loc = "upper right", ncol = 2)
plt.savefig("itint_rand_am_comp.png", bbox_inches = 'tight') 
plt.close(fig)

# Plot Hedging Strategy
Htrue = np.loadtxt("itint_hedg_am_strat.csv", delimiter = ";")
Htrue = np.reshape(Htrue[Mtrain:], (M-Mtrain, n-1, d))
indplot = np.random.choice(np.arange(M-Mtrain), 2, replace = False)
for i in range(d):
    if i % 2 == 0:
        ylim = [-1.1, 0.3]
    else:
    ...
    fig = plt.figure()
    plt.plot(tt[:-1], Htrue[indplot[0], :, i], "k-")
    for NN in range(1, NNN+1):
        plt.plot(tt[:-1], Hpred[NN, indplot[0], :, i], linestyle = ":", color = cmap(NN/NNN), label = "N = " + str(NN))   
        
    plt.plot(tt[:-1], Htrue[indplot[1], :, i], "k-")
    for NN in range(1, NNN+1):
        plt.plot(tt[:-1], Hpred[NN, indplot[1], :, i], linestyle = ":", color = cmap(NN/NNN))  
    ...
    plt.legend(loc = "upper right", ncol = 4)
    plt.savefig("itint_rand_am_hedgetest_" + str(i+1) + ".png", bbox_inches = 'tight') 
    plt.close(fig)