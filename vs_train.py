...
X = np.reshape(np.loadtxt("vs/vs_X.csv", dtype = np.float32), [L, K, d])
X0 = X[0:1, 0:1]
sigma = np.reshape(np.loadtxt("vs/vs_sigma.csv", dtype = np.float32), [L, K, 1])
G = np.loadtxt("vs/vs_G.csv", dtype = np.float32)
...
for n in range(N):
    b1 = time.time()
    WphiT = np.sum(phi[:, :, n]*np.expand_dims(X[:, :-1]/X0, axis = 2), axis = -1)
    Wphi0 = np.sum(phi[:, 0:1, n]*np.expand_dims(X[:, 0:1]/X0, axis = 2), axis = -1)
    WphiI = np.cumsum(np.sum(derphi[:, :, n]*np.expand_dims(X[:, :-1]/X0, axis = 2), axis = -1)*dt, axis = 1)
...
for n in range(N+1):
    ...
    np.savetxt("vs/vs_res_" + str(n) + ".csv", loss)
    np.savetxt("vs/vs_cmp_" + str(n) + ".csv", comp)
    
    np.savetxt("vs/vs_Gpr_" + str(n) + ".csv", Gpred)
    np.savetxt("vs/vs_Hpr_" + str(n) + ".csv", np.reshape(Hpred, [L, -1]))
    del Gpred, Hpred