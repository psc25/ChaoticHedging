...
X12 = np.loadtxt("vs/vs_X.csv", delimiter = ";", dtype = np.float32)
X12 = np.reshape(X12, [L, K, 2])
X = X12[:, :, 0]
sigma = X12[:, :, 1]
G = np.loadtxt("vs/vs_G.csv", delimiter = ";", dtype = np.float32)
...
for N in range(NN+1):
    ...
    if N == 0:
        ...
        for n in range(N):
            ...
            Qg = np.sum(np.expand_dims(np.square(rnd_n[:-1, n]), 0)*np.expand_dims(sigma[:, :-1], -1)*dt, axis = 1)
        ...
        for n in range(N):
            ...
            Qg = np.cumsum(np.expand_dims(np.square(rnd_n[:-1, n]), 0)*np.expand_dims(sigma[:, :-1], -1)*dt, axis = 1)
    ...
    np.savetxt("vs/vs_res_" + str(N) + ".csv", loss, delimiter = ";")
    np.savetxt("vs/vs_cmp_" + str(N) + ".csv", comp, delimiter = ";")
    
    np.savetxt("vs/vs_Gpr_" + str(N) + ".csv", Gpred, delimiter = ";")
    np.savetxt("vs/vs_Hpr_" + str(N) + ".csv", Hpred, delimiter = ";")